...
import { initAudio, getMuted, toggleMuted } from '@/components/AudioEngine';
import {
  Camera,
  Grid,
  AlignJustify,
...
import { getFrameTheme, persistFrameTheme, readPersistedFrameId, slotsForLayout } from '@/lib/frames';
...
function CaptureRoom() {
  ...
  const frameTheme = getFrameTheme(searchParams.get('frame'));

  const [appLoaded, setAppLoaded] = useState(false);
  const [step, setStep] = useState<AppStep>('capture');
  const [layout, setLayout] = useState<LayoutType>(frameTheme.layout);
  ...
  // Read the persisted mute preference after mount so SSR markup stays stable
  useEffect(() => {
    setSoundMuted(getMuted());
  }, []);

  // Restore a remembered frame if the URL is missing one (after a refresh)
  useEffect(() => {
    const fromUrl = searchParams.get('frame');
    if (!fromUrl) {
      const saved = readPersistedFrameId();
      if (saved) router.replace(`/booth?frame=${saved}`);
      return;
    }
    persistFrameTheme(fromUrl);
    setLayout(getFrameTheme(fromUrl).layout);
  }, [searchParams, router]);

  // Target count of photos to select based on layout
  const targetCount = slotsForLayout(layout);
      ...
      <header className="w-full border-b border-[#E8DFCE] bg-[#FDFBF7]/90 backdrop-blur-md sticky top-0 z-30 shadow-xs">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <Link
              href="/"
              aria-label="Back to frame selection"
              className="w-8 h-8 bg-zinc-900 rounded-lg flex items-center justify-center text-white shadow-sm shrink-0 transition-transform hover:scale-105 active:scale-95"
            >
              <ArrowLeft size={16} />
            </Link>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline text-[9px] font-bold tracking-wider px-1.5 py-0.5 rounded bg-amber-100 text-amber-900 border border-amber-200">
                  gethaloluna.com
                </span>
              </div>
              <p className="text-[10px] text-zinc-500 font-mono uppercase tracking-widest mt-0.5 truncate">
                Capture Room
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <button
              ...
              aria-label={soundMuted ? 'Unmute booth sounds' : 'Mute booth sounds'}
              ...
              className={`w-9 h-9 rounded-full border flex items-center justify-center transition-all active:scale-95 ${
                soundMuted
                  ...
                  : 'bg-white/80 border-[#E8DFCE] text-zinc-700 hover:border-zinc-400 shadow-xs'
              }`}
            >
            ...
            <Link
              href="/"
              className="group flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-full border border-[#E8DFCE] bg-white/80 shadow-xs transition-all hover:border-zinc-400 hover:shadow-sm"
            >
              <span
                className="w-6 h-6 rounded-full flex items-center justify-center text-[13px] leading-none shrink-0"
                style={{ backgroundColor: frameTheme.wash }}
              >
                {frameTheme.emoji}
              </span>
              <span className="flex flex-col items-start leading-none">
                <span className="text-[11px] font-bold text-zinc-800">{frameTheme.name}</span>
                <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-400 group-hover:text-zinc-600 transition-colors">
                  Change frame
                </span>
              </span>
            </Link>
        ...
        {step === 'edit' && (
          <div className="bg-[#FAF7F0] border border-[#E8DFCE] rounded-2xl p-5 sm:p-6 shadow-sm w-full">
            <CanvasEditor
              ...
              layout={layout}
              initialPreset={frameTheme.preset}
...
export default function BoothPage() {
  return (
    <Suspense fallback={<div className="min-h-screen w-full bg-[#FDFBF7]" />}>
      <CaptureRoom />
    </Suspense>
  );
}
